...
test('a sync completed during startup still refreshes a potentially stale first read', async () => {
  const f = fixture()
  const signals = []
  f.monitor.subscribeRefresh(signal => signals.push(signal))
  f.respond({ running: false, last_sync_at: new Date(Date.now() + 1000).toISOString(), sources: [] })
  const stop = f.monitor.start()
  await tick()
...
test('an old idle startup baseline leaves the initial data request running', async () => {
  const f = fixture()
  const signals = []
  f.monitor.subscribeRefresh(signal => signals.push(signal))
  f.respond({ running: false, last_sync_at: '2020-01-01T00:00:00Z', sources: [] })
  const stop = f.monitor.start()
  await tick()
